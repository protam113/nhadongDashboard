...
export const resizeFile = async (file: File): Promise<File> => {
  return new Promise((resolve, reject) => {
      // Nếu tệp là PDF, không cần xử lý, trả về luôn
            ...
            if (blob) {
              ...
              if (blob.size > 2 * 1024 * 1024) {  // > 2MB
                ...
                canvas.toBlob(
                  (lowerQualityBlob) => {
                    if (lowerQualityBlob) {
                      resolve(new File([lowerQualityBlob], file.name, {
                        type: file.type,
