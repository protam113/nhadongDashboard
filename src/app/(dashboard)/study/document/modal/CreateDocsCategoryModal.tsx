...
const CreateDocsCategoryModal: React.FC = () => {
    ...
    const handleSubmit = () => {
        ...
        createCategory({ name, model: "document", image: imageList[0]?.originFileObj ?? null });

        // Check user role and show appropriate message
        if (userRoleId === 1) { // Admin
            message.success("Tạo Thể Loại Document Thành Công!");
        } else if (userRoleId === 2) { // Manager
            message.warning("Tạo Thể Loại Document Thành Công! Vui lòng đợi admin duyệt.");
        } else {
            message.warning("Tạo Thể Loại Document Thành Công! Chờ admin duyệt.");
        }

        router.push("/study/document"); // Redirect after submission
    };
...
export default CreateDocsCategoryModal;
